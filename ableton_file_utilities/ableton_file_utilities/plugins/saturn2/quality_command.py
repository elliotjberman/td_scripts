"""CLI for switching FabFilter Saturn 2 quality modes in Ableton sets."""
...
from ableton_file_utilities.plugins.saturn2 import vst3
...
def change_file(
    ...
    target_mode = vst3.canonical_mode(mode)
    ...
    return ChangeReport(
        ...
        target_label=vst3.QUALITY_LABELS[target_mode],
        dry_run=dry_run,
...
def patch_xml(xml: str, target_mode: str) -> tuple[str, list[DeviceReport]]:
    ...
    for start, end in live_set.iter_plugin_device_ranges(xml):
        block = xml[start:end]
        if not vst3.is_saturn2_block(block):
            continue
        result = vst3.patch_block(block, target_mode)
        reports.append(
            DeviceReport(
...
def format_report(report: ChangeReport) -> str:
    ...
    lines = [
        ...
        f"FabFilter Saturn 2 devices found: {report.devices_seen}",
...
def format_device(device: DeviceReport) -> list[str]:
    ...
    lines.append(f"  {marker}: ProcessorState@{vst3.QUALITY_OFFSET}: {device.old_value} -> {device.new_value}")
    return lines


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Switch FabFilter Saturn 2 VST3 quality mode in an Ableton .als file.")
    parser.add_argument("session", type=Path, help="Path to an Ableton .als file.")
    parser.add_argument("--mode", required=True, help="Target mode: normal, high-quality, or super-high-quality.")
